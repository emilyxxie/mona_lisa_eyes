...
function max(args) {
    ...
    if (maxInputIsTransposed) {
        ...
        var xTransposed = transpose({ inputs: { x: x }, attrs: { perm: permutedAxes }, backend: backend });
        if (backend.dataIdMap.get(xTransposed.dataId).id !== xId) {
            // If perm is not no-op.
            var xTransposedVals = backend.typedArrayFromHeap(xTransposed);
    ...
    var _a = tfjsCore.backend_util.computeOutAndReduceShapes(xShape, axes), outShape = _a[0], reduceShape = _a[1];
...
function select(args) {
    var inputs = args.inputs, backend = args.backend;
    var _a = inputs, condition = _a.condition, t = _a.t, e = _a.e;
...
function split(args) {
    var inputs = args.inputs, attrs = args.attrs, backend = args.backend;
    var x = inputs.x;
    var _a = attrs, numOrSizeSplits = _a.numOrSizeSplits, axis = _a.axis;
